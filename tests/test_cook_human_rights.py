...
def test_description():
    assert parsed_items["description"] == (
        "Special Meeting of the Cook County Commission on Human Rights"
    )
...
def test_time_notes():
    assert parsed_items["time_notes"] == (
        "Regular meetings are held on the second Thursday of every other month"
    )


def test_id():
    assert parsed_items["id"] == (
        "cook_human_rights/201907161130/x/commission_on_human_rights"
    )
...
def test_source():
    assert parsed_items["source"] == (
        "https://www.cookcountyil.gov/event/"
        "cook-county-commission-human-rights-meeting-3"
    )
...
def test_links():
    ...
    assert parsed_items["links"] == [
        {
            "href": (
                "https://www.cookcountyil.gov/sites/default/files/"
                "july_16_2019_special_meeting_minutes_-_draft_-_sep_6_2019.pdf"
            ),
            "title": "Minutes",
        }
    ]
