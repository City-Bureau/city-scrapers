...
test_response = file_response(
    ...
    url=('https://www.portauthority.org/paac/CompanyInfoProjects/'
...
def test_id():
    assert (parsed_items[0]['id'] ==
            'alle_port_authority/201801260930/x/annual_meeting')
...
def test_sources():
    assert parsed_items[0]['sources'] == [{
        'note': '',
        'url': ('https://www.portauthority.org/paac/'
...
def test_documents():
    assert (parsed_items[0]['documents'][0] ==
            {'note': 'Minutes',
             'url': ('http://www.portauthority.org/paac/'
