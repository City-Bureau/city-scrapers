...
from city_scrapers.spiders.chi_ward_night import Calendar, ChiWardNightSpider
...
def test_start():
    assert parsed_items[0]['start'] == {'date': date(2017, 11, 7), 'time': time(16, 00)}


def test_end():
    assert parsed_items[0]['end'] == {'date': date(2017, 11, 7), 'time': time(18, 00)}
...
def test_weekly_generation():
    row = [
        'Gregory Mitchell', '773.731.7777', 'test@example.com',
        'https://www.gregmitchell7thward.org/', '7', 'Bridget Newsham', 'Yes',
        '2249 E 95th St. Chicago ', 'Weekly', 'Monday', '3:00 PM', '7:00 PM', 'No', '',
    ...
    assert events[0]['end'] == {'date': date(2017, 11, 6), 'time': time(19, 00)}
    ...
    assert events[1]['end'] == {'date': date(2017, 11, 13), 'time': time(19, 00)}
    ...
    assert events[2]['end'] == {'date': date(2017, 11, 20), 'time': time(19, 00)}
...
def test_monthly_generation():
    row = [
        ...
        '',
        '',
        'Every fourth Tuesday, no meeting in November or December, group meetings rather than one-on-one'  # noqa
    ]
    ...
    assert events[0]['start'] == {'date': date(2017, 11, 28), 'time': time(18, 00)}
    assert events[0]['end'] == {'date': date(2017, 11, 28), 'time': time(20, 00)}
    assert events[0]['id'] == 'chi_ward_night/201711281800/x/ward_night_ward_5'

    assert events[1]['start'] == {'date': date(2017, 12, 26), 'time': time(18, 00)}
    assert events[1]['end'] == {'date': date(2017, 12, 26), 'time': time(20, 00)}
    assert events[1]['id'] == 'chi_ward_night/201712261800/x/ward_night_ward_5'

    assert events[2]['start'] == {'date': date(2018, 1, 23), 'time': time(18, 00)}
    assert events[2]['end'] == {'date': date(2018, 1, 23), 'time': time(20, 00)}
...
def test_third_thursday():
    ...
    assert days == [date(2017, 10, 19), date(2017, 11, 16), date(2017, 12, 21), date(2018, 1, 18)]
...
def test_every_monday():
    ...
    assert days == [date(2017, 10, 16), date(2017, 10, 23), date(2017, 10, 30), date(2017, 11, 6)]
...
def test_last_friday():
    ...
    assert days == [date(2017, 10, 27), date(2017, 11, 24), date(2017, 12, 29), date(2018, 1, 26)]
