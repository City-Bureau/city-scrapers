...
class ChiLicenseAppealSpider(CityScrapersSpider):
    ...
    def _parse_meetings(self, response):
        ...
        needs.
        """
        response_text = " ".join(response.css(".page-full-description *::text").extract())
        self._validate_location(response_text)
        year_str = re.search(r"\d{4}",
                             response.css("h1.page-heading::text").extract_first()).group()
        time_str = re.search(r"\d{1,2}:\d{2} [apm\.]{2,4}", response_text).group().replace(".", "")
    ...
    def _parse_start(self, item_text, time_str, year_str):
        ...
        date_str = re.sub(r"\s+", " ", re.search(r"[a-zA-Z]{3,10}\s+\d{1,2}", item_text).group())
    ...
    def _parse_links(self, item, response):
        """
        Parse or generate links. No links are currently present, but it's similar to other pages
        where they are sometimes posted inside the li tag.
        """
        links = []
