...
class ChiSsa27bSpider(CityScrapersSpider):
    name = "chi_ssa_27b"
    ...
    def parse_committee(self, respon, url):
        ...
        for i3 in cleaned:
            ...
            elif s1 in i3:  # strong meeting time section
                item_txt = Selector(text=i3).css("p::text").get()
                the_href = Selector(text=i3).css('a::attr(href)').get()

                if '2019' in item_txt:
                    words = [("Sept","Sep"),("June", "Jun"),("am", "AM"),("pm", "PM"),(".",""),("\xa0",""),("-","")]
                    for tup in words:
                    ...
                    print("!----final str: ", str(item_txt),'!')
                    #p_idx = max(item_txt.find(' AM'), item_txt.find(' PM'), 0) + 4  # so we can slice
                    #time_str = item_txt[:p_idx]                                        # remove comments from the rest of the string
