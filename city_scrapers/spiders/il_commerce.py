...
class IlCommerceSpider(CityScrapersSpider):
    ...
    start_urls = ["https://www.icc.illinois.gov/meetings/default.aspx?dts=32&et=1&et=5&et=3"]
    ...
    def parse(self, response):
        ...
        for nav_link in response.css(".col-sm-7 a.btn"):
            if "?bd=" in nav_link.attrib["href"]:
                yield response.follow(nav_link.attrib["href"], callback=self._parse_events_page)
    ...
    def _parse_title(self, response):
        ...
        return re.sub(
            r"(Illinois Commerce Commission|(?=Committee )Committee Meeting$)", "", title_str
        ).strip()

    def _parse_description(self, response):
        """Parse or generate meeting description."""
        return re.sub(r"\s+", " ",
                      " ".join(response.css(".col-sm-12 > p *::text").extract())).strip()
    ...
    def _parse_start(self, response):
        ...
        dt_str = re.search(r"[A-Z][a-z]{2,8} \d{1,2}, \d{4} \d{1,2}:\d{2} [APM]{2}",
                           start_str).group()
    ...
    def _parse_location(self, response):
        ...
        addr_items = [i.strip() for i in location_items if "Building" not in i and i.strip()]
        name_items = [i.strip() for i in location_items if "Building" in i and i.strip()]
        return {
