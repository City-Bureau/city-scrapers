...
class ChiPlanCommissionSpider(CityScrapersSpider):
    ...
    def parse(self, response):
        ...
        for meeting_group in response.css(".page-full-description table[cellspacing]"):
            year_str = meeting_group.xpath("preceding::strong[1]/text()").re_first(r"\d{4}")
                    ...
                    if len(detail_links) > 0:
                        for link in detail_links:
                            yield response.follow(
                                link, callback=self._parse_detail, cb_kwargs={"start": start}
                            )
                        continue
                    meeting = Meeting(
                        title='Commission',
    ...
    def _parse_detail_start(self, response, start):
        detail_text = " ".join(response.css(".col-xs-12 > p *::text").extract())
        time_strs = re.findall(r"(\d{1,2}(\:\d{2})?\s+[apm\.]{2,4})", detail_text, flags=re.I)
        time_objs = []
    ...
    def _parse_detail_links(self, response):
        ...
        for link in response.css(".page-full-description-above a"):
            title_str = re.sub(r"\s+", " ", " ".join(link.css("*::text").extract())).strip()
            if (
                "mailto" in link.attrib["href"]
                or link.attrib["href"].endswith("/chicago_plan_commission.html") or not title_str
            ):
                continue
