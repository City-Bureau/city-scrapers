...
class ChiDevelopmentFundSpider(Spider):
            ...
            for meeting in meetings:
                ...
                data = {
                    ...
                    'documents': self._parse_documents(
                        column, meeting, response
                    ),
                }
                data['end'] = {
                    'date': data['start']['date'], 'time': None, 'note': ''
                }
    ...
    def parse_description(response):
        desc_xpath = (
            ')//p[contains(text(), '
            '"The Chicago City Council established")]//text()'
        )
        description = ' '.join(
            t.strip() for t in response.xpath(desc_xpath).extract()
        )
        return description
    ...
    def _parse_start(meeting):
        ...
        try:
            dt, other_text = dateutil.parser.parse(
                meeting, fuzzy_with_tokens=True
            )
